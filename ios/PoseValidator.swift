...
class PoseValidator {
    ...
    struct PostureAccuracy {
        let shoulderAccurateLeft: Bool
        let shoulderAccurateRight: Bool
        let elbowAccurateLeft: Bool
        let elbowAccurateRight: Bool
        let spineAccurate: Bool
        let hipAccurateLeft: Bool
        let hipAccurateRight: Bool
    }
    ...
    func isValidPose(_ landmarks: PoseLandmarks, 
    ...
    func calculatePostureMetrics(_ landmarks: PoseLandmarks) -> PostureMetrics? {
    ...
    // Compare with reference and return detailed accuracy
    func compareWithReference(_ metrics: PostureMetrics) -> PostureAccuracy {
        let shoulderAccurateLeft = abs(metrics.shoulderAngleLeft - referencePose.shoulderAngle) < referencePose.tolerance
        let shoulderAccurateRight = abs(metrics.shoulderAngleRight - referencePose.shoulderAngle) < referencePose.tolerance
        let elbowAccurateLeft = abs(metrics.elbowAngleLeft - referencePose.elbowAngle) < referencePose.tolerance
        let elbowAccurateRight = abs(metrics.elbowAngleRight - referencePose.elbowAngle) < referencePose.tolerance
        let spineAccurate = metrics.spineAngle < referencePose.spineAngle + referencePose.tolerance
        let hipAccurateLeft = abs(metrics.hipAngleLeft - referencePose.hipAngle) < referencePose.tolerance
        let hipAccurateRight = abs(metrics.hipAngleRight - referencePose.hipAngle) < referencePose.tolerance
